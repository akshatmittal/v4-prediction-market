...
contract TradeLimitHookTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        (address hookAddress, bytes32 salt) = HookMiner.find(
            address(this),
            flags,
            type(TradeLimitHook).creationCode,
            abi.encode(address(manager))
        );

        limitHook = new TradeLimitHook{salt: salt}(
            IPoolManager(address(manager))
        );
        require(
            address(limitHook) == hookAddress,
            "MarketHookTest: hook address mismatch"
        );
    ...
    function test_launchFull() public {
        ...
        console2.log("Creating Prediction Market contracts...");
        ...
        for (uint256 i = 0; i < toApprove.length; i++) {
            ...
            predictionMarket.outcomeTokens(0).approve(
                toApprove[i],
                Constants.MAX_UINT256
            );
            predictionMarket.outcomeTokens(1).approve(
                toApprove[i],
                Constants.MAX_UINT256
            );
        }
        ...
        (currency0_, currency1_) = SortTokens.sort(
            MockERC20(address(predictionMarket.outcomeTokens(0))),
            MockERC20(address(collateral))
        );
        PoolKey memory keyP1 = PoolKey(
            currency0_,
            currency1_,
            3000,
            1,
            IHooks(address(limitHook))
        );
        (currency0_, currency1_) = SortTokens.sort(
            MockERC20(address(predictionMarket.outcomeTokens(1))),
            MockERC20(address(collateral))
        );
        PoolKey memory keyP2 = PoolKey(
            currency0_,
            currency1_,
            3000,
            1,
        ...
        console2.log("Minting outcome tokens...");
        vm.startPrank(user);
        ...
        for (uint256 i = 0; i < 2; ++i) {
            ...
            console2.log(
                "Current user balance of outcome token ",
                i,
                ":",
                outcomeToken.balanceOf(user)
            );
        }

        console2.log("Swapping outcome tokens (0 for 1)...");
        ...
        swapRouter.swap(
            ...
            IPoolManager.SwapParams({
                zeroForOne: keyP1.currency0 ==
                    Currency.wrap(address(predictionMarket.outcomeTokens(0))),
        ...
        for (uint256 i = 0; i < 2; ++i) {
            OutcomeToken outcomeToken = predictionMarket.outcomeTokens(i);
            console2.log(
                "Final user balance of outcome token ",
                i,
                ":",
                outcomeToken.balanceOf(user)
            );
        ...
        assertGt(
            predictionMarket.outcomeTokens(1).balanceOf(user),
            100 * 10 ** 6
        );
