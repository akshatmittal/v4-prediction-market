...
contract TradeLimitHookTest is Test, Deployers {
    ...
    function test_launchFull() public {
        ...
        manager.initialize(keyP1, SQRT_PRICE_1_1, abi.encode(address(predictionMarket)));
        manager.initialize(keyP2, SQRT_PRICE_1_1, abi.encode(address(predictionMarket)));
        ...
        for (uint256 i = 0; i < 2; ++i) {
            ...
            console2.log("Current user balance of outcome token ", i, ":", outcomeToken.balanceOf(user));
        }
        ...
        for (uint256 i = 0; i < 2; ++i) {
            OutcomeToken outcomeToken = predictionMarket.outcomeTokens(i);
            console2.log("Final user balance of outcome token ", i, ":", outcomeToken.balanceOf(user));
        }
