...
contract Market is MarketData {
    ...
    constructor(
        ...
        // Validate inputs
        require(
            address(_eventContract) != address(0) &&
    ...
    function fund(
        uint _funding
    ) public isCreator atStage(Stages.MarketCreated) {
        require(
            eventContract.collateralToken().transferFrom(
                msg.sender,
                address(this),
                _funding
            ) &&
                eventContract.collateralToken().approve(
        ...
        for (uint8 i = 0; i < outcomeCount; i++)
            require(
                eventContract.outcomeTokens(i).transfer(
                    creator,
                    eventContract.outcomeTokens(i).balanceOf(address(this))
                )
            );
    ...
    /// @return fees amount
    function withdrawFees() public isCreator returns (uint fees) {
    ...
    function buy(
        ...
        uint outcomeTokenCount,
        uint maxCost
    ) public atStage(Stages.MarketFunded) returns (uint cost) {
        ...
        (int netCost, int outcomeTokenNetCost, uint fees) = tradeImpl(
            outcomeCount,
            outcomeTokenAmounts,
            int(maxCost)
        );
    ...
    function sell(
        ...
        uint outcomeTokenCount,
        uint minProfit
    ) public atStage(Stages.MarketFunded) returns (uint profit) {
    ...
    function shortSell(
        ...
        eventContract.outcomeTokens(outcomeTokenIndex).approve(
            address(this),
            outcomeTokenCount
        );
        uint profit = this.sell(
            outcomeTokenIndex,
            outcomeTokenCount,
            minProfit
        );
    ...
    function trade(
        ...
        int collateralLimit
    ) public atStage(Stages.MarketFunded) returns (int netCost) {
        uint8 outcomeCount = eventContract.getOutcomeCount();
        require(outcomeTokenAmounts.length == outcomeCount);

        int outcomeTokenNetCost;
        uint fees;
        (netCost, outcomeTokenNetCost, fees) = tradeImpl(
            outcomeCount,
            outcomeTokenAmounts,
            collateralLimit
        );
    ...
    function tradeImpl(
        ...
        int[] memory outcomeTokenAmounts,
        int collateralLimit
    ) private returns (int netCost, int outcomeTokenNetCost, uint fees) {
        // Calculate net cost for executing trade
        outcomeTokenNetCost = marketMaker.calcNetCost(
            this,
            outcomeTokenAmounts
        );
        if (outcomeTokenNetCost < 0)
            fees = calcMarketFee(uint(-outcomeTokenNetCost));
        else fees = calcMarketFee(uint(outcomeTokenNetCost));

        require(int(fees) >= 0);
        netCost = outcomeTokenNetCost + (int(fees));
        ...
        uint allowance = eventContract.collateralToken().allowance(
            msg.sender,
            address(this)
        );
        ...
        if (outcomeTokenNetCost < 0) {
            ...
            if (netCost < 0) {
                require(
                    eventContract.collateralToken().transfer(
                        msg.sender,
                        uint(-netCost)
                    )
                );
