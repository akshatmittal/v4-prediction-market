...
contract MarketMaker {
    ...
    function calcNetCost(
        Market market,
        int[] memory outcomeTokenAmounts
    ) public view returns (int netCost) {
        ...
        int log2N = Fixed192x64Math.binaryLog(
            netOutcomeTokensSold.length * ONE,
            Fixed192x64Math.EstimationMode.UpperBound
        );

        uint funding = market.funding();

        int costLevelBefore = calcCostLevel(
            log2N,
            netOutcomeTokensSold,
            funding,
            Fixed192x64Math.EstimationMode.LowerBound
        );
    ...
    function calcCost(
        ...
        netOutcomeTokensSold[outcomeTokenIndex] =
            netOutcomeTokensSold[outcomeTokenIndex] +
            (int(outcomeTokenCount));
        // Calculate cost level after balance was updated
    ...
    function calcMarginalPrice(
        ...
        require(market.eventContract().getOutcomeCount() > 1);
        int[] memory netOutcomeTokensSold = getNetOutcomeTokensSold(market);
        int logN = Fixed192x64Math.binaryLog(
            netOutcomeTokensSold.length * ONE,
        ...
        (uint sum, , uint outcomeExpTerm) = sumExpOffset(
            logN,
            netOutcomeTokensSold,
            funding,
            outcomeTokenIndex,
    ...
    function calcCostLevel(
        int logN,
        int[] memory netOutcomeTokensSold,
        uint funding,
        Fixed192x64Math.EstimationMode estimationMode
    ) private pure returns (int costLevel) {
        ...
        (uint sum, int offset, ) = sumExpOffset(
            logN,
            netOutcomeTokensSold,
            funding,
            0,
        ...
        costLevel = ((costLevel * (int(ONE))) / logN) * (int(funding));
    }
    ...
    function sumExpOffset(
        ...
        uint8 outcomeIndex,
        Fixed192x64Math.EstimationMode estimationMode
    ) private pure returns (uint sum, int offset, uint outcomeExpTerm) {
        ...
        offset = (offset * (logN)) / int(funding);
        offset = offset - (EXP_LIMIT);
        uint term;
        for (uint8 i = 0; i < netOutcomeTokensSold.length; i++) {
            term = Fixed192x64Math.pow2(
                ((netOutcomeTokensSold[i] * (logN)) / int(funding)) - (offset),
                estimationMode
            );
    ...
    function getNetOutcomeTokensSold(
        Market market
    ) private view returns (int[] memory quantities) {
